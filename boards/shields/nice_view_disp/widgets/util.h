...
struct status_state {
    /* Compatibility fields used by the stock peripheral-only screen. */
    uint8_t battery;
    bool charging;

    /* Local battery is the central half (right on this Corne). */
    uint8_t local_battery;
    bool local_charging;

#if !IS_ENABLED(CONFIG_ZMK_SPLIT) || IS_ENABLED(CONFIG_ZMK_SPLIT_ROLE_CENTRAL)
    /* Battery reported by the split peripheral (left half). */
    uint8_t peripheral_battery;
    bool peripheral_battery_available;
